# -*- coding: utf-8 -*-
"""
Standalone parameter editor for HollowCubeInflation.py.
...
_BOOL_FIELDS = [
    ...
    ('Exploit Z symmetry (halves FEM elements -- NOT valid here: the '
     'base is clamped only at the bottom, not top+bottom)', 'symmetry_z', False),
    ('Prevent nodes crossing symmetry planes (one-sided stop, no collision model; '
     'guards all X/Y/Z midplanes regardless of whether their symmetry is exploited)',
     'prevent_symmetry_crossing', True),
    ...
    ('Mirror visual model across Z symmetry plane', 'mirror_visual_z', True),
...
def _count_fem_elements(params):
    ...
    symmetry_x/y/z flag -- one half extended by symmetry_buffer_blocks
    extra cells past the plane), shell cells being all cells in that
    sub-range minus the unmeshed interior (cavity) ones. Mirrors the
    axis-resolution and range logic in HollowCubeInflation.py's
    ...
    def axis_range(divs, symmetric, keep_lower_half, buf):
        if not symmetric:
            return 0, divs
        return (0, min(divs, divs // 2 + buf)) if keep_lower_half else (max(0, divs // 2 - buf), divs)
    ...
    divs_z, wall_z = resolve_axis(params['height_blocks'], params['wall_height_blocks'], params['symmetry_z'])

    i_lo, i_hi = axis_range(divs_x, params['symmetry_x'], False, buf)
    j_lo, j_hi = axis_range(divs_y, params['symmetry_y'], False, buf)
    k_lo, k_hi = axis_range(divs_z, params['symmetry_z'], True, buf)
    ...
    return (total_cells - interior_cells) * 6


class ParamsEditor(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("HollowCubeInflation parameters")
